function reg = MultiPolyRegressM(Data,R,PW,varargin)
...
%   Copyright (c) 2015, Ahmet Cecen  -  All rights reserved.
    ...
    if nargin > 3
        for ii=1:length(varargin)
            ...
            if strcmp(varargin{ii},'none') == 1
                NormalizationSwitch='None';
            end
...
%     A
    ...
    T = evalc(['PolyExp = ',variablesexp,Poly]);
    ...
    elseif strcmp(NormalizationSwitch,'None')==1
        mae = mean(abs(r));
        maestd = std(abs(r));
    else
        mae = mean(abs(r./R));
